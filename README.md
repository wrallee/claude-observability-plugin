...
| `LANGFUSE_CAPTURE_IMAGES` | Upload images to Langfuse and show them in the trace (default `true`). Set to `false` to keep image markers without uploading media. Takes precedence over `CC_LANGFUSE_CAPTURE_IMAGES`. | No |
| `CC_LANGFUSE_CAPTURE_IMAGES` | Legacy Claude-specific image capture setting; used when `LANGFUSE_CAPTURE_IMAGES` is unset. | No |
